...
export class Version0033to0034 {
	...
	public upgrade = ( currentVersion: number ) => {
		return new Promise(( resolve, reject ) => {
			...
			if ( currentVersion > expectedCurrentVersion ) {
				...
				this.db.query( 'ALTER TABLE `logs` ADD COLUMN `refid` BIGINT UNSIGNED NOT NULL DEFAULT 0 AFTER `details`', ( err, results, fields ) => {
					if ( err ) {
						if ( err.code === 'ER_DUP_FIELDNAME' ) {
							this.db.query( 'ALTER TABLE logs MODIFY refid BIGINT UNSIGNED NOT NULL DEFAULT 0 AFTER details', ( ierr, iresults, ifields ) => {
								if ( ierr ) {
									reject( ierr );
								} else {
