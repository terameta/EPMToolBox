...
export const DimeTagActions = {
	ALL: {
		LOAD: {
			...
			complete: ( payload: DimeTag[] ): Action => ( { type: DimeTagActions.ALL.LOAD.COMPLETE, payload: payload } )
		}
	},
	ONE: {
		LOAD: {
			...
			complete: ( payload: DimeTag ): Action => ( { type: DimeTagActions.ONE.LOAD.COMPLETE, payload: payload } )
		},
		CREATE: {
			INITIATE: 'DIME_TAG_ACTIONS_ONE_CREATE_INITIATE',
			initiate: ( payload: DimeTag ): Action => ( { type: DimeTagActions.ONE.CREATE.INITIATE, payload: payload } ),
			COMPLETE: 'DIME_TAG_ACTIONS_ONE_CREATE_COMPLETE',
			complete: ( payload: DimeTag ): Action => ( { type: DimeTagActions.ONE.CREATE.COMPLETE, payload: payload } )
		},
		UPDATE: {
			INITIATE: 'DIME_TAG_ACTIONS_ONE_UPDATE_INITIATE',
			initiate: ( payload: DimeTag ): Action => ( { type: DimeTagActions.ONE.UPDATE.INITIATE, payload: payload } ),
			COMPLETE: 'DIME_TAG_ACTIONS_ONE_UPDATE_COMPLETE',
			complete: ( payload: DimeTag ): Action => ( { type: DimeTagActions.ONE.UPDATE.COMPLETE, payload: payload } )
		},
		DELETE: {
