...
export class Effects {
	...
	@Effect() ALL_LOAD_INITIATE_IF_EMPTY$ = this.actions$.pipe(
		...
		, filter( ( [action, store] ) => ( !store.stream.items || Object.keys( store.stream.items ).length === 0 ) )
	...
	@Effect() ONE_CREATE_COMPLETE$ = this.actions$.pipe(
		...
		, switchMap( ( action: Action ) => {
			this.router.navigateByUrl( 'admin/streams/stream-detail/' + action.payload.id );
	...
	@Effect() ONE_LOAD_INITIATE_IF_EMPTY$ = this.actions$.pipe(
		...
		, withLatestFrom( this.store$ )
		, filter( ( [action, store] ) => ( !store.stream.curItem || store.stream.curItem.id === 0 || store.stream.curItem.id !== parseInt( action.payload, 10 ) ) )
		, map( ( [action, store] ) => action )
	...
	@Effect() ONE_DELETE_COMPLETE$ = this.actions$.pipe(
		...
		, switchMap( ( action: Action ) => {
			this.router.navigateByUrl( 'admin/streams/stream-list' );
			return of( DimeStreamActions.ALL.LOAD.initiate() );
		} ) );
