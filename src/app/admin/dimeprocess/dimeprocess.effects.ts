...
export class Effects {
	...
	@Effect() ALL_LOAD_INITIATEIFEMPTY$ = this.actions$.pipe(
		...
		, filter( ( [action, state] ) => ( !state.process.items || Object.keys( state.process.items ).length === 0 ) )
	...
	@Effect() ONE_CREATE_COMPLETE$ = this.actions$.pipe(
		...
		, switchMap( ( action: Action<DimeProcess> ) => {
			this.router.navigateByUrl( 'admin/processes/process-detail/' + action.payload.id );
	...
	@Effect() ONE_LOAD_INITIATEIFEMPTY$ = this.actions$.pipe(
		...
		, withLatestFrom( this.store$ )
		, filter( ( [action, state] ) => ( !state.process.curItem || state.process.curItem.id === 0 || state.process.curItem.id !== action.payload ) )
		, map( ( [action, state] ) => action )
	...
	@Effect() ONE_ISPREPARED_INITIATE$ = this.actions$.pipe(
		...
		, switchMap( ( action: Action<number> ) => {
			return this.backend.isPrepared( action.payload ).pipe(
				map( resp => DimeProcessActions.ONE.ISPREPARED.COMPLETE.action( resp ) )
				, catchError( resp => of( DimeStatusActions.error( resp, this.serviceName ) ) ) );
		} ) );

	@Effect() ONE_STEP_LOADALL_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_STEP_UPDATEALL_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_STEP_UPDATEALL_COMPLETE$ = this.actions$.pipe(
		ofType( DimeProcessActions.ONE.STEP.UPDATEALL.COMPLETE.type )
		, withLatestFrom( this.store$ )
		, map( ( [action, state] ) => DimeProcessActions.ONE.STEP.LOADALL.INITIATE.action( state.process.curItem.id ) ) );

	@Effect() ONE_STEP_CREATE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_STEP_UPDATE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_STEP_UPDATE_COMPLETE$ = this.actions$.pipe(
		ofType( DimeProcessActions.ONE.STEP.UPDATE.COMPLETE.type )
		, withLatestFrom( this.store$ )
		, map( ( [action, state] ) => DimeProcessActions.ONE.STEP.LOADALL.INITIATE.action( state.process.curItem.id ) ) );

	@Effect() ONE_STEP_DELETE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_STEP_DELETE_COMPLETE$ = this.actions$.pipe(
		ofType( DimeProcessActions.ONE.STEP.DELETE.COMPLETE.type )
		, withLatestFrom( this.store$ )
		, map( ( [action, state] ) => DimeProcessActions.ONE.STEP.LOADALL.INITIATE.action( state.process.curItem.id ) ) );

	@Effect() ONE_DEFAULTTARGETS_LOAD_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_DEFAULTTARGETS_UPDATE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_FILTERS_LOAD_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_FILTERS_UPDATE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_FILTERSDATAFILE_LOAD_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_FILTERSDATAFILE_UPDATE_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_RUN_INITIATE$ = this.actions$.pipe(
	...
	@Effect() ONE_CHECKLOG_INITIATE$ = this.actions$.pipe(
		ofType( DimeProcessActions.ONE.CHECKLOG.INITIATE.type )
		, switchMap( ( action: Action<number> ) => {
			return this.backend.checkLog( action.payload ).pipe(
				map( resp => DimeProcessActions.ONE.CHECKLOG.COMPLETE.action( resp ) )
				, catchError( resp => of( DimeStatusActions.error( resp, this.serviceName ) ) ) );
		} ) );

	@Effect( { dispatch: false } ) ONE_CHECKLOG_COMPLETE$ = this.actions$.pipe(
		ofType( DimeProcessActions.ONE.CHECKLOG.COMPLETE.type )
		, withLatestFrom( this.store$ )
		, filter( ( [action, state] ) => ( state.process.curItem.status === DimeProcessStatus.Running ) )
		, map( ( [action, state] ) => action )
	...
	@Effect() ONE_SENDDATAFILE_INITIATE$ = this.actions$.pipe(
		...
		, switchMap( action => {
			return this.backend.sendDataFile( action.payload ).pipe(
				map( resp => DimeProcessActions.ONE.SENDDATAFILE.COMPLETE.action( action.payload ) )
				, catchError( resp => of( DimeStatusActions.error( resp, this.serviceName ) ) ) );
		} ) );
	@Effect() ONE_SENDDATAFILE_COMPLETE$ = this.actions$.pipe(
