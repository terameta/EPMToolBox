...
export const DimeTagGroupActions = {
	ALL: {
		LOAD: {
			...
			complete: ( payload: TagGroup[] ): Action => ( { type: DimeTagGroupActions.ALL.LOAD.COMPLETE, payload: payload } )
		}
	},
	ONE: {
		LOAD: {
			...
			complete: ( payload: TagGroup ): Action => ( { type: DimeTagGroupActions.ONE.LOAD.COMPLETE, payload: payload } )
		},
		CREATE: {
			INITIATE: 'DIME_TAGGROUP_ACTIONS_ONE_CREATE_INITIATE',
			initiate: ( payload: TagGroup ): Action => ( { type: DimeTagGroupActions.ONE.CREATE.INITIATE, payload: payload } ),
			COMPLETE: 'DIME_TAGGROUP_ACTIONS_ONE_CREATE_COMPLETE',
			complete: ( payload: TagGroup ): Action => ( { type: DimeTagGroupActions.ONE.CREATE.COMPLETE, payload: payload } )
		},
		UPDATE: {
			INITIATE: 'DIME_TAGGROUP_ACTIONS_ONE_UPDATE_INITIATE',
			initiate: ( payload: TagGroup ): Action => ( { type: DimeTagGroupActions.ONE.UPDATE.INITIATE, payload: payload } ),
			COMPLETE: 'DIME_TAGGROUP_ACTIONS_ONE_UPDATE_COMPLETE',
			complete: ( payload: TagGroup ): Action => ( { type: DimeTagGroupActions.ONE.UPDATE.COMPLETE, payload: payload } )
		},
		DELETE: {
