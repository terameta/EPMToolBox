...
export class Effects {
	...
	@Effect() ALL_LOAD_INITIATE_IF_EMPTY$ = this.actions$.pipe(
		...
		, filter( ( [action, state] ) => ( !state.matrix.items || Object.keys( state.matrix.items ).length === 0 ) )
	...
	@Effect() ONE_CREATE_COMPLETE$ = this.actions$.pipe(
		...
		, switchMap( ( action: Action<DimeMatrix> ) => {
			this.router.navigateByUrl( 'admin/matrices/matrix-detail/' + action.payload.id );
	...
	@Effect() ONE_LOAD_INITIATEIFEMPTY$ = this.actions$.pipe(
		...
		, withLatestFrom( this.store$ )
		, filter( ( [action, state] ) => ( !state.matrix.curItem || state.matrix.curItem.id === 0 || state.matrix.curItem.id !== action.payload ) )
		, map( ( [action, state] ) => action )
